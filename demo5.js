...
async function saveModel() {
  if  ( selectModelNum == 0 ) {
    await model.save('localstorage://model_demo5_');
  } else {
    await model.save('localstorage://model_demo5_' + selectModelNum);
  }
}
...
async function loadModel() {
  try {
    if  ( selectModelNum == 0 ) {
      model = await tf.loadLayersModel('localstorage://model_demo5_');
    } else {
      model = await tf.loadLayersModel('localstorage://model_demo5_' + selectModelNum);
    }
  } catch(e) {
     document.querySelector('#console').textContent =
...
function peakDbThresholdAndPositionCheck(data, threshold) {
  ...
  function peakCheck(peakOld, num) {
    var peakNew = peakOld;
    if ( num > peakOld ) {
      peakNew = num;
    } else {
  ...
  for( var index = 0; index < 43; index++) {
    ...
    //console.log(index, currentPeak, peak, mainPeakIndex );
    var peakChange = currentPeak - peak;
  ...
  // Display peak
  document.getElementById('currentVolume').textContent = lastPeak.toFixed(1);
...
function reset() {
  document.getElementById('leftRightOutput').value = 5;
  document.getElementById('upDownOutput').value    = 5;
}
...
async function moveSlider(labelTensor) {
 ...
 if ( label == 0 || label == 1 ) {
   const prevValue = +document.getElementById('leftRightOutput').value;
   document.getElementById('leftRightOutput').value =
      prevValue + (label === 0 ? -delta : delta);
  } else {
    const prevValue = +document.getElementById('upDownOutput').value;
    if ( label == 3 ) {
      document.getElementById('upDownOutput').value = prevValue + delta;
    } else {
      document.getElementById('upDownOutput').value = prevValue - delta;
    }
  }
...
async function displayLabel(labelTensor) {
 ...
 document.getElementById("dispButton3").style.color = "green";
 document.getElementById("dispButton4").style.color = "green";
 ...
 if ( label == 0 ) {
   ...
   document.getElementById("dispButton3").style.color = "red";
 } else if ( label == 4 ) {
   document.getElementById("dispButton4").style.color = "red";
 }
}
